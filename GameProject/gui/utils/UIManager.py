...
class UIManager:
    ...
    WIDGET_MAP = {
        ...
        "GameToggleButton": GameToggleButton,

        # === ДОДАЄМО НАШІ КОМПОНЕНТИ В МАПУ ===
    ...
    def add(
        ...
        **props,
        ...
        if widget_id in self.registry or widget_id in self.structure:
            raise ValueError(f"Widget '{widget_id}' already exists")

        # зберігаємо опис структури (як і раніше)
        self.structure[widget_id] = {
            "type": widget_type,
            "parent": parent,
            "props": props,
            "style_class": style_class,
        }
        self.children_map.setdefault(parent, []).append(widget_id)
        if parent and parent not in self.structure:
            self._ensure_container(parent)

        if bounds:
            x1, y1, x2, y2 = bounds
            self.set_bounds(widget_id, x1, y1, x2, y2)

        return widget_id

    # === ДОДАТКОВО (не обов'язково) — утиліти для зміни стану ===
    def set_toggle_active(self, widget_id: str, active: bool):
        tb = self._widgets[widget_id]
        tb.state = "down" if active else "normal"

    def is_toggle_active(self, widget_id: str) -> bool:
        tb = self._widgets[widget_id]
        return tb.state == "down"
